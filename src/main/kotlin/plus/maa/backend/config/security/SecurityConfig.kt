...
class SecurityConfig(
    ...
    fun filterChain(http: HttpSecurity): SecurityFilterChain {
        //关闭CSRF,设置无状态连接
        http.csrf { obj: CsrfConfigurer<HttpSecurity> -> obj.disable() } //不通过Session获取SecurityContext
        ...
        http.authorizeHttpRequests { authorize ->
            authorize.requestMatchers(*URL_WHITELIST).anonymous()
                .requestMatchers(*URL_PERMIT_ALL).permitAll() //权限 0 未激活 1 激活  等等.. (拥有权限1必然拥有权限0 拥有权限2必然拥有权限1、0)
                //指定接口需要指定权限才能访问 如果不开启RBAC注释掉这一段即可
                .requestMatchers(*URL_AUTHENTICATION_1).hasAuthority("1") //此处用于管理员操作接口
    ...
    companion object {
        ...
        private val URL_PERMIT_ALL = arrayOf(
            "/",
            "/error",
            "/version",
        ...
        //添加需要权限1才能访问的接口
        private val URL_AUTHENTICATION_1 = arrayOf(
            "/copilot/delete",
            "/copilot/update",
            "/copilot/upload",
            "/comments/add",
        ...
        private val URL_AUTHENTICATION_2 = arrayOf(
            "/file/download/**",
            "/file/download/",
            "/file/disable",
            "/file/enable",
